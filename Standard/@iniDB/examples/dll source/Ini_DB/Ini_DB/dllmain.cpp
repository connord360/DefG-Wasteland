...
#define INIDB_VERSION_NUMBER "1.0"
...
extern "C"
{
	__declspec(dllexport) void __stdcall RVExtension(char *output, int outputSize, const char *function)
	{
		...
		//logger.write("inidb.log", "function (%s)", function);
			...
			if(func)
			{
				if(_stricmp(func, "read") == 0)
				{
					...
					if(file != NULL && sec != NULL && key != NULL)
					{
						...
						if(Ini::Read(logger.dirFile(realFile), sec, key, "c0f916b469c17e0f967c6774e0d837fac0f916b469c17e0f967c6774e0d837fa", trueValue, 512))
						{
				...
				else if(_stricmp(func, "write") == 0)
				{
					...
					if(file != NULL && sec != NULL && key != NULL && val != NULL)
					{
						...
						//logger.write("inidb.log", "write[%s, %s, %s, %s]", logger.dirFile(realFile), sec, key, val);
				...
				else if(_stricmp(func, "exists") == 0)
				{
					...
					if(file != NULL)
					{
						...
						//logger.write("inidb.log", "exists[%s]", logger.dirFile(realFile));
				...
				{
					//logger.write("inidb.log", "Unknown Command [%s]", func);
